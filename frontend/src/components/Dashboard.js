...
const getPackage = async (packageID) => {
  try {
      const response = await axios.get(`https://adventureally.onrender.com/packages/`+packageID);
...
const getTravellers = async (bookingID) => {
  try {
      const response = await axios.get(`https://adventureally.onrender.com/booking/travellers/`+bookingID);
      return response.data;
  } catch (error) {
      throw error;
  }
...
function Dashboard() {
  ...
  const fetchUserTrips = async () => {
    try {
     const bookings  = (await axios.get('https://adventureally.onrender.com/booking/'+localStorage.getItem('userName'))).data;
